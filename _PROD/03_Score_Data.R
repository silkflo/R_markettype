...
sbx_macd <- file.path(path_sbx, paste0("AI_Macd", chart_period,"-300.csv"))
...
if(exists("df_temp") && !file.exists(file.path(path_data, "macd_ai_classified.rds")))
  {
    # write file first time
    write_rds(df_temp, file.path(path_data, "macd_ai_classified.rds"))
} else if(exists("df_temp")) {
    # read previous file
      read_rds(file.path(path_data, "macd_ai_classified.rds")) %>% 
      # join obtained data
      bind_rows(df_temp) %>% 
      # write data back
      write_rds(file.path(path_data, "macd_ai_classified.rds"))
